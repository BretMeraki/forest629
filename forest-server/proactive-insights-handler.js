...
const logger = getForestLogger({ module: 'ProactiveInsightsHandler' });
...
export class ProactiveInsightsHandler {
  constructor(dataPersistence, projectManagement, eventBus = null) {
    ...
    if (process.stdin.isTTY) {
      logger.info(
        '🧠 ProactiveInsightsHandler initialized - Ready to transform insights into wisdom'
      );
    }
  }
  ...
  setupEventListeners() {
    ...
    this.eventBus.on(
      ...
      this.handleStrategicInsights.bind(this),
      'ProactiveInsightsHandler'
    );
    this.eventBus.on(
      'system:risks_detected',
      this.handleRiskDetection.bind(this),
      'ProactiveInsightsHandler'
    );
    this.eventBus.on(
    ...
    this.eventBus.on(
      ...
      this.handleIdentityInsights.bind(this),
      'ProactiveInsightsHandler'
    );
  }
  ...
  async handleStrategicInsights({ projectId, insights, systemState, analysisType, analyzedAt }) {
    try {
      logger.info('📊 Processing strategic insights...');

      if (!insights?.insights || insights.insights.length === 0) {
        logger.debug('📊 No strategic insights to process');
        return;
      }
      ...
      logger.info(
        `✨ Processed ${processedInsights.length} strategic insights, generated ${recommendations.length} recommendations`
      );
    } catch (error) {
      logger.error('❌ Error handling strategic insights', { error, projectId });
      await this.dataPersistence.logError(
        'ProactiveInsightsHandler.handleStrategicInsights',
        error
      );
    }
  }
  ...
  async handleRiskDetection({ projectId, risks, riskLevel, detectedAt }) {
    try {
      logger.info(`⚠️ Processing ${risks.length} detected risks (level: ${riskLevel})...`);

      if (risks.length === 0) {
        logger.debug('⚠️ No risks detected');
        return;
      }
      ...
      logger.info(
        `🚨 Processed ${risks.length} risks, ${criticalRisks.length} critical, generated ${mitigationStrategies.length} mitigation strategies`
      );
    } catch (error) {
      logger.error('❌ Error handling risk detection', { error, projectId, riskLevel });
  ...
  async handleOpportunityDetection({ projectId, opportunities, priorityLevel, detectedAt }) {
    try {
      logger.info(
        `🎯 Processing ${opportunities.length} detected opportunities (priority: ${priorityLevel})...`
      );
      ...
      const actionPlans = await this.generateOpportunityActionPlans(
        enrichedOpportunities,
        projectId
      );

      // Identify time-sensitive opportunities
      const urgentOpportunities = enrichedOpportunities.filter(
        o => o.timeWindow && (o.timeWindow.includes('immediate') || o.timeWindow.includes('next 2'))
      );
      ...
      logger.info(
        `🌟 Processed ${opportunities.length} opportunities, ${urgentOpportunities.length} urgent, generated ${actionPlans.length} action plans`
      );
    } catch (error) {
      logger.error('❌ Error handling opportunity detection', {
        error,
        projectId,
        priorityLevel,
      });
      await this.dataPersistence.logError(
        'ProactiveInsightsHandler.handleOpportunityDetection',
        error
      );
    }
  }
  ...
  async handleIdentityInsights({ projectId, identityInsights, reflectedAt }) {
    try {
      logger.info(`🧘 Processing identity insights for project ${projectId}...`);

      if (!identityInsights || Object.keys(identityInsights).length === 0) {
        logger.debug('🧘 No identity insights to process');
        return;
      }
      ...
      // Emit the action plan
      if (identityActionPlan) {
        this.eventBus.emit('insights:identity_action_plan_available', identityActionPlan);

        logger.info(
          `🌟 Processed identity insights, generated ${
            Array.isArray(identityActionPlan.actions) ? identityActionPlan.actions.length : 0
      ...
      logger.info(
        `🌟 Processed identity insights, generated ${
          identityActionPlan?.actions.length || 0
        } identity-based actions`
      );
    } catch (error) {
      logger.error('❌ Error handling identity insights', { error, projectId });
  ...
  async generateStrategicRecommendations(processedInsights, projectId) {
    ...
    for (const insight of processedInsights) {
      ...
      if (this.lastRecommendations.has(recentKey)) {
        ...
        if (hoursSince < 24) {
          continue;
        } // Don't repeat recommendations within 24 hours
      }
    ...
    switch (insight.type) {
      case 'trajectory_accelerating':
        return {
          ...
          recommendation:
            ...
            'Capitalize on your excellent momentum by tackling more ambitious challenges.',
          actions: [
      ...
      case 'trajectory_stalling':
        return {
          ...
          recommendation:
            insight.actionSuggestion ||
            'Re-engage with smaller, achievable tasks to rebuild momentum.',
          actions: [
      ...
      case 'momentum_high_velocity':
        return {
          ...
          recommendation:
            insight.actionSuggestion ||
            'Maintain this exceptional pace while monitoring sustainability.',
          actions: [
      ...
      case 'momentum_low_velocity':
        return {
          ...
          recommendation:
            insight.actionSuggestion ||
            'Address barriers to consistent learning and rebuild momentum.',
          actions: [
  ...
  identifyUrgentInsights(processedInsights) {
    return processedInsights.filter(
      insight =>
  ...
  async generateRiskMitigationStrategies(prioritizedRisks, projectId) {
    ...
    for (const risk of prioritizedRisks.slice(0, 3)) {
      // Top 3 risks
  ...
  async generateOpportunityActionPlans(enrichedOpportunities, projectId) {
    ...
    for (const opportunity of enrichedOpportunities.slice(0, 2)) {
      // Top 2 opportunities
  ...
  async generateProactiveAlerts(projectId, items, type) {
    ...
    logger.info(`🚨 Generated proactive alert: ${alertTitle}`);
  }
  ...
  calculateInsightUrgency(insight, systemState) {
    ...
    if (insight.type.includes('stalling') || insight.type.includes('declining')) {
      urgency += 2;
    }

    // High momentum insights are moderately urgent (opportunity cost)
    if (insight.type.includes('accelerating') || insight.type.includes('momentum')) {
      urgency += 1;
    }
  ...
  calculateStrategicValue(insight) {
    ...
    if (insight.type.includes('momentum') || insight.type.includes('trajectory')) {
      value += 1;
    }
  ...
  calculateOpportunityUrgency(opportunity) {
    if (opportunity.timeWindow?.includes('immediate')) {
      return 3;
    }
    if (opportunity.timeWindow?.includes('next 2') || opportunity.timeWindow?.includes('next 3')) {
      return 2;
    }
    return 1;
  }
  ...
  async storeInsightHistory(projectId, type, insights) {
    try {
      ...
      logger.error('Error storing insight history', { error, projectId, type });
    }
  }
  ...
  async storeProactiveAlert(projectId, alert) {
    try {
      ...
      logger.error('Error storing proactive alert', { error, projectId });
    }
  }
...
logger.debug('ProactiveInsightsHandler module loaded');
