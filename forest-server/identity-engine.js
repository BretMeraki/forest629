...
export class IdentityEngine {
  ...
  extractSkillAreas(completedTopics, skillTaxonomy = null) {
    const skillAreas = {};

    // Use provided taxonomy or infer from topics
    const taxonomy = skillTaxonomy || this.inferSkillTaxonomy(completedTopics);

    for (const topic of completedTopics) {
      const text = topic.topic.toLowerCase();

      // Categorize skills using dynamic taxonomy
      for (const [category, keywords] of Object.entries(taxonomy)) {
        if (keywords.some(keyword => text.includes(keyword.toLowerCase()))) {
          skillAreas[category] = (skillAreas[category] || 0) + 1;
        }
      }
    }
  ...
  inferSkillTaxonomy(completedTopics) {
    // Dynamically infer skill categories from completed topics
    const topicWords = completedTopics
      .map(t => t.topic.toLowerCase())
      .join(' ')
      .split(/\s+/)
      .filter(word => word.length > 3);

    // Create dynamic categories based on common patterns
    const taxonomy = {
      primary: [],
      secondary: [],
      tertiary: []
    ...
    // Extract unique meaningful words as potential skill indicators
    const uniqueWords = [...new Set(topicWords)];

    // Distribute words across categories based on frequency
    const wordFreq = {};
    topicWords.forEach(word => {
      wordFreq[word] = (wordFreq[word] || 0) + 1;
    });

    const sortedWords = uniqueWords.sort((a, b) => (wordFreq[b] || 0) - (wordFreq[a] || 0));

    // Assign to categories based on frequency
    taxonomy.primary = sortedWords.slice(0, Math.ceil(sortedWords.length / 3));
    taxonomy.secondary = sortedWords.slice(Math.ceil(sortedWords.length / 3), Math.ceil(2 * sortedWords.length / 3));
    taxonomy.tertiary = sortedWords.slice(Math.ceil(2 * sortedWords.length / 3));
  ...
  inferProfessionalRole(config, completedTopics) {
    ...
    // Extract role from goal context dynamically
    const goalWords = goal.split(/\s+/).filter(word => word.length > 3);
    const taskWords = taskPatterns.split(/\s+/).filter(word => word.length > 3);

    // Find the most frequent meaningful word that could indicate a role
    const allWords = [...goalWords, ...taskWords];
    const wordFreq = {};
    allWords.forEach(word => {
      wordFreq[word] = (wordFreq[word] || 0) + 1;
    });

    // Get the most common meaningful word as potential role indicator
    const mostCommonWord = Object.entries(wordFreq)
      .sort(([,a], [,b]) => b - a)
      .find(([word]) => word.length > 4)?.[0];

    if (mostCommonWord) {
      return `${mostCommonWord}_practitioner`;
    }
  ...
  extractTargetRole(goal) {
    // Extract role dynamically from goal text
    const goalWords = goal.toLowerCase().split(/\s+/).filter(word => word.length > 4);

    // Find words that might indicate a target role
    const roleIndicators = goalWords.filter(word =>
      !['want', 'need', 'like', 'would', 'could', 'should', 'will', 'develop', 'learn'].includes(word)
    );

    if (roleIndicators.length > 0) {
      // Use the first meaningful role indicator
      return `${roleIndicators[0]}_practitioner`;
    }
  ...
  extractTargetSkillLevel(goal, metrics) {
    ...
    // Look for skill level indicators in goal and metrics
    if (goalText.includes('expert') || goalText.includes('advanced') || goalText.includes('mastery')) {
      return 'expert';
    }
    if (goalText.includes('intermediate') || metricsText.includes('revenue') || metricsText.includes('client')) {
      return 'intermediate';
    }
    return 'competent';
  }
  ...
  extractTargetResourceLevel(metrics) {
    const hasFinancialMetrics = metrics.some(
      m => m.includes('revenue') || m.includes('$') || m.includes('earnings')
    );
  ...
  extractTargetBeliefs(goal) {
    ...
    if (goal.includes('expert') || goal.includes('advanced')) {
      beliefs.push('I am capable of mastery');
    }
    if (goal.includes('advanced') || goal.includes('career')) {
      beliefs.push('I am skilled in my field');
    }

    // Default empowering beliefs
    beliefs.push('I can reach my goals');
  ...
  extractTargetReputation(goal, metrics) {
    ...
    if (
      goal.includes('advanced') ||
      metrics.some(m => m.includes('showcase') || m.includes('clients'))
    ) {
  ...
  generateMicroShifts(currentIdentity, targetIdentity, transformationProgress) {
    ...
    microShifts.push({
      ...
      current: 'I am working to develop X',
      target: 'I am already developing X through my actions',
      action: 'Act as if you already are developing your target identity',
    });
  ...
  analyzeIdentityBarriers(currentIdentity, targetIdentity, completedTopics) {
    ...
    if (currentIdentity.networkPosition.position === 'isolated') {
      barriers.push({
        ...
        description: 'Limited network connections',
  ...
  calculateNextMilestone(currentIdentity, targetIdentity) {
    ...
    const gaps = [
      ...
      { type: 'network', gap: networkGap, milestone: 'Make first meaningful connections' },
      {
        type: 'confidence',
        gap: confidenceGap,
  ...
  generateMilestoneActions(milestoneType) {
    const actions = {
      ...
      confidence: [
        'Set and reach small, clear goals',
  ...
  analyzeStrategicPositioning(currentIdentity, targetIdentity, htaData) {
    ...
    if (currentRole !== targetRole && confidenceLevel >= 4) {
      ...
      return {
        ...
        title: 'Established Advanced Identity',
